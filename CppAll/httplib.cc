...
bool Server::handle_file_request(Request &req, Response &res,
  ...
  for (const auto &kv : base_dirs_) {
	...
	qDebug() << "request file: " << req.path.c_str();
    // Prefix match
...
bool Server::dispatch_request(Request &req, Response &res,
  ...
  try {
    ...
    res.status = 500;
	qDebug() << "error happened:"<< req.path.c_str() <<" \nerror reason:" << ex.what();
...
bool Client::write_request(Stream &strm, const Request &req,
  ...
  // Body
  if (req.body.empty()) {
	  //由于传入了 cont_provider 所以是这儿
    if (req.content_provider) {
      size_t offset = 0;
      size_t end_offset = req.content_length;

      DataSink data_sink;//这里是接收数据的对象 data_sink的body 本身是一个string
      data_sink.write = [&](const char *d, size_t l) {
		  //它的write 是直接调用stream 进行写 但是一次写入多少是没有规定的
